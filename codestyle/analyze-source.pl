...
sub filter_program {
    ...
    if ($child1 == 0) {
        ...
        my @output = <$fh>;
        close($fh); # or print("$0: close: $! @program $fh $text");
        return @output;
    }
}
...
foreach my $file (@filelist)
{
    ...
    elsif ($file =~ m!external|Debug|Release|test|external_tools|.git|cmake|codestyle/!) {
    ...
    elsif ($file =~ /.*\.(h|cc|cpp|hpp|h.in)$/) {
        process_cpp($file);
    }
    elsif ($file =~ /\.(pl|pro)$/) {
        process_pl_cmake($file);
    }
    elsif ($file =~ m!(^|/)LICENSE$!) {
      #  process_pl_cmake($file);
    }
    elsif ($file =~ m!(^|/)README\.md$!) {
      #  process_pl_cmake($file);
    }
    elsif ($file =~ m!(^|/)compile_commands\.json$!) {
      #  process_pl_cmake($file);
    }
