...
int RBC::ScanAndBcast(const void* sendbuf, void* recvbuf_scan, 
        void* recvbuf_bcast, int count, MPI_Datatype datatype,
        MPI_Op op, RBC::Comm const &comm) {
    int tag = RBC::Tag_Const::SCANANDBCAST;
    int rank, size;
    ...
    for (int cur_height = height; cur_height > 0; --cur_height) {
        if (cur_height == down_height) {
            ...
            RBC::Sendrecv(tmp_buf, count * 2, datatype, target, tag, 
 ...
 */
class Range_Requests::IscanAndBcast : public RBC::R_Req {
public:
    IscanAndBcast(const void *sendbuf, void *recvbuf_scan, void *recvbuf_bcast,
            int count, MPI_Datatype datatype, int tag, MPI_Op op, RBC::Comm const &comm);
    ~IscanAndBcast();
...
int RBC::IscanAndBcast(const void* sendbuf, void* recvbuf_scan, void* recvbuf_bcast,
